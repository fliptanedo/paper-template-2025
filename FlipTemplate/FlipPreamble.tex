...
\usepackage[nopatch=footnote]{microtype}% re: footnote warning 
...
\usepackage{mathrsfs}				% Weinberg-y letters
\usepackage{bbm}					  % \mathbbm{1} conflict: XeLaTeX 
\usepackage{cancel}					% cross out
\usepackage[normalem]{ulem} % for \sout
\usepackage{youngtab}	    	% Young Tableaux
\usepackage{mleftright}     % \mleft, \mright; bracket size/space
% \usepackage{nicefrac}       % frac in subscript; use xfrac
\usepackage{xfrac}       % frac in subscript
...
% \usepackage{nicefrac}   % frac. in tables; calld xfrac above
\usepackage{multirow}   % multirow elements in a table
% \usepackage{arydshln}   % dashed lines, causes errors DEPRECATED; use nicematrix
...
%% if you use etoc for chapter TOC, you end up using firststyle
